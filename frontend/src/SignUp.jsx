...
function SignUp() {
    ...
    useEffect(() => {
        const initPage = async () => {
            ...
            // If there's an active session, pre-fill the email
            if (session?.user) {
                setEmail(session.user.email);
            }

            // If no session and no recovery token, we might want to redirect
            // unless this is a pure "New User" registration page
            if (!session && (!window.location.hash || !window.location.hash.includes('access_token'))) {
                // navigate('/LoginPage'); // Optional: uncomment if you want to force login first
            }
    ...
    const handleSignUp = async (e) => {
        ...
        if (password !== passwordConfirm) {
            setPopupConfig({
                isOpen: true, type: 'error', title: 'Passwords Mismatch',
        ...
        if(!isStrong){
            setPopupConfig({
                isOpen: true, type: 'error', title: 'Password is weak',
        ...
        try {
            ...
            // 1. Add user to your custom database tables
            await FetchData.AddUser(email, password, firstname, surname, role, orgID, deptID);
            
            // 2. Refresh session to ensure app knows the user is registered
            ...
            setPopupConfig({
                ...
                message: error.code === '23505' ? "Email already in use." : (error.message || "Something went wrong.")
            });
        } finally {
                ...
                <form onSubmit={handleSignUp} className="space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className={INPUT_CONTAINER}>
                                    ...
                                    <input type="text" value={firstname} placeholder="Jane" required className={INPUT_STYLE} onChange={(e) => setFirstname(e.target.value)} />
                                </div>
                            </div>
                            <div className="space-y-1">
                                ...
                                <div className={INPUT_CONTAINER}>
                                    <User size={18} className={ICON_STYLE} />
                                    <input type="text" value={surname} placeholder="Doe" required className={INPUT_STYLE} onChange={(e) => setSurname(e.target.value)} />
                                </div>
                            </div>
                            <div className="space-y-1">
                                ...
                                <div className={INPUT_CONTAINER}>
                                    ...
                                    <input type="email" value={email} placeholder="jane.doe@school.edu" required className={INPUT_STYLE} onChange={(e) => setEmail(e.target.value)} />
                                </div>
                            </div>
                        </div>

                        {/* Column 2: Professional Details & Password */}
                        <div className="space-y-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className={INPUT_CONTAINER}>
                                    ...
                                    <select value={role} onChange={(e) => setRole(e.target.value)} className={INPUT_STYLE} required>
                            ...
                            <div className="space-y-1">
                                ...
                                <div className={INPUT_CONTAINER}>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => { setSelectedSchool(e.target.value); getALlDepartments(e.target.value); }}
                                    ...
                                    >
                                        ...
                                        {schools.map((school) => (
                                            <option key={school.id || school.OrganisationID} value={school.id || school.OrganisationID}>
                                                {school.Name}
                                            </option>
                                        ))}
                                    </select>
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="mt-2">
                                <div className="h-1 w-full bg-gray-200 rounded-full">
                                    <div className={`h-1 transition-all duration-300 rounded-full ${isStrong ? 'w-full bg-green-500' : 'w-1/3 bg-red-400'}`}></div>
                                </div>
                                <p className={`text-[10px] mt-1 font-medium ${isStrong ? 'text-green-600' : 'text-gray-400'}`}>
                                    {isStrong ? "✓ Meets requirements" : "Requires: 6+ chars, 1 Capital, 1 Special"}
                                </p>
                            </div>
                        </div>
                        <div className="space-y-1">
                    ...
                    <div className="pt-6 space-y-4">
                        <button
                            ...
                            className={`w-full flex items-center justify-center gap-3 py-4 px-6 rounded-xl font-bold text-white transition-all duration-200 shadow-md ${isSubmitting ? 'bg-gray-400' : 'bg-blue-600 hover:bg-blue-700'}`}
                        >
                            {isSubmitting ? "Processing..." : "Create Account"}
                        </button>
                        <Link to="/LoginPage" className="w-full flex items-center justify-center gap-2 py-3 px-4 text-gray-500 font-semibold hover:bg-gray-100 rounded-xl">
                            <ArrowLeft size={18} /> Already have an account? Log in
                        </Link>
                    </div>
                </form>
            </div>
